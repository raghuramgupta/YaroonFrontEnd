...
const MessagesInbox = () => {
    ...
    if (userId) {
      ...
      const interval = setInterval(() => {
        setOnlineStatus(prev => ({
          ...prev,
          [selectedConversation?.partnerId]: Math.random() > 0.3 // 70% chance to be online
        }));
      }, 30000);
  ...
  useEffect(() => {
    scrollToBottom();
    if (selectedConversation && groupedMessages[selectedConversation.key]) {
      // Mark messages as read when opening conversation
      groupedMessages[selectedConversation.key].forEach(msg => {
      ...
      if (!partnerInfo[partnerId]) {
        fetchUserInfo(partnerId);
      }
      
      // Fetch property info if not already available
      if (selectedConversation.listingId && !propertyInfo[selectedConversation.listingId]) {
        fetchPropertyInfo(selectedConversation.listingId);
      }
    }
  ...
  const fetchPropertyInfo = async (propertyId) => {
    try {
      const response = await fetch(`${config.apiBaseUrl}/api/listings/${propertyId}`);
      if (response.ok) {
  ...
  const fetchMessages = async (userId) => {
    try {
      ...
      if (receivedRes.ok && sentRes.ok) {
        ...
        const grouped = groupConversations([...received, ...sent], userId);
        setGroupedMessages(grouped);
        if (!selectedConversation && Object.keys(grouped).length > 0) {
          setSelectedConversation({
            key: Object.keys(grouped)[0],
            partnerId: grouped[Object.keys(grouped)[0]][0].senderId === userId 
              ? grouped[Object.keys(grouped)[0]][0].receiverId 
              : grouped[Object.keys(grouped)[0]][0].senderId,
            listingId: grouped[Object.keys(grouped)[0]][0].listingId
          });
        }
      } else {
  ...
  const groupConversations = (msgs, currentUserId) => {
    const groups = {};
    
    msgs.forEach((msg) => {
      // Create a unique key for the conversation combining both users and the listing
      const partnerId = msg.senderId === currentUserId ? msg.receiverId : msg.senderId;
      const conversationKey = msg.listingId 
        ? `${partnerId}-${msg.listingId}` 
        : partnerId;
      
      if (!groups[conversationKey]) groups[conversationKey] = [];
      groups[conversationKey].push(msg);
    });
    
    // Sort each conversation by timestamp
    Object.keys(groups).forEach(conversationKey => {
      groups[conversationKey].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
    });
    ...
    try {
      ...
      if (response.ok) {
        ...
        setGroupedMessages(prevGrouped => {
          ...
          for (const conversationKey in updated) {
  ...
  const filteredGroupedMessages = Object.entries(groupedMessages)
    .map(([conversationKey, msgs]) => {
      const partnerId = msgs[0].senderId === currentUser ? msgs[0].receiverId : msgs[0].senderId;
      ...
      <div className="messenger-app">
        ...
        <div className="messenger-container">
          ...
          <div className="conversation-sidebar">
            ...
            <div className="conversation-list">
              {filteredGroupedMessages.length === 0 ? (
                ...
                filteredGroupedMessages.map(({conversationKey, partnerId, listingId, messages}) => {
                  ...
                  const property = propertyInfo[listingId] || { title: 'Property' };
                  
                  return (
                    <div
                      key={conversationKey}
                      className={`conversation-item ${
                        selectedConversation?.key === conversationKey ? 'active' : ''
                      }`}
                      ...
                      <div className="conversation-details">
                        <div className="conversation-header">
                          <span className="partner-name">
                            {listingId ? property.title : partner.name}
                          </span>
                        ...
                        {listingId && (
                          <div className="property-preview">
                            ...
                            <span>{property.propertyAddress || 'Property'}</span>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div className="chat-area">
            {selectedConversation && groupedMessages[selectedConversation.key] ? (
              <>
                      ...
                      <div className="partner-avatar">
                        <div className="avatar">
                          {selectedConversation.listingId ? (
                            ...
                            partnerInfo[selectedConversation.partnerId]?.name?.charAt(0).toUpperCase() || 
                            selectedConversation.partnerId.charAt(0).toUpperCase()
                          )}
                        </div>
                        {onlineStatus[selectedConversation.partnerId] && <div className="online-badge"></div>}
                      </div>
                      <div className="partner-details">
                        <h3>
                          {selectedConversation.listingId 
                            ? propertyInfo[selectedConversation.listingId]?.title || 'Property'
                            : partnerInfo[selectedConversation.partnerId]?.name || selectedConversation.partnerId}
                        </h3>
                        <p className="status">
                          {onlineStatus[selectedConversation.partnerId] ? 'Online' : 'Offline'}
                        </p>
                      </div>
                    </div>
                  ...
                  {selectedConversation.listingId && (
                    <div className="property-info-header">
                      <div className="property-info-content">
                        ...
                        <span>
                          {propertyInfo[selectedConversation.listingId]?.propertyAddress || 'Property address not available'}
                        </span>
                      </div>
                      <a 
                ...
                <div className="message-container">
                  <div className="messages">
                    {groupedMessages[selectedConversation.key].map((msg) => {
